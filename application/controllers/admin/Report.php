...
class Report extends CI_Controller
{
    ...
    public function export($start_date = 0, $end_date = 0)
    {

        // $transaksi = $this->transaksi_model->get_alltransaksi();
        $transaksi = $this->transaksi_model->get_report_excel($start_date, $end_date);
        // var_dump($start_date);
        // die;
        ...
        foreach ($transaksi as $transaksi) {
            ...
            $spreadsheet->setActiveSheetIndex(0)
                ...
                ->setCellValue('F' . $kolom, $transaksi->grand_total);
        ...
        $total_transaksi = $this->transaksi_model->total_transaksi_excel($start_date, $end_date);
        // var_dump($total_transaksi);
        // die;

        $spreadsheet->setActiveSheetIndex(0)
            ->setCellValue('F' . $kolom, $total_transaksi->total_price);


        $writer = new Xlsx($spreadsheet);

        header('Content-Type: application/vnd.ms-excel');
        header('Content-Disposition: attachment;filename="Laporan Tanggal "' . "$start_date" . ' " sampai Tanggal "' . "$end_date" . '".xlsx"');
