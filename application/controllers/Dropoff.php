...
class Dropoff extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->input->post('kota_asal') != NULL) {
            $kota_id = $this->input->post('kota_asal');
            $this->session->set_userdata(array("kota_asal" => $kota_id));
        } else {
            if ($this->session->userdata('kota_asal') != NULL) {
                $kota_id = $this->session->userdata('kota_asal');
            }
        }
        $kota_tujuan = "";
        if ($this->input->post('kota_tujuan') != NULL) {
            $kota_id = $this->input->post('kota_tujuan');
            $this->session->set_userdata(array("kota_tujuan" => $kota_id));
        } else {
            if ($this->session->userdata('kota_tujuan') != NULL) {
                $kota_id = $this->session->userdata('kota_tujuan');
            }
        }
        $tanggal_sewa = "";
    ...
    public function paket($kota_id = false, $mobil_id = false)
    {
        ...
        $paket      = $this->paket_model->get_paket_dropoff($kota_id, $mobil_id);
        // var_dump($paket);
        // die;
        $data = [
            'title'         => 'Pilih Kendaraan',
